...
def add_song_genre(file_name, table_name):
    ...
    join_data["artist_genre"] = join_data["artist_genre"].apply(lambda x: x.split(",") if isinstance(x, str) else [])
...
def main(logical_date):
    add_song_genre(f'join_artist_info_track10_{logical_date}.csv', 'ARTIST_INFO_TOP10')
    add_song_genre(f'join_artsit_info_chart_{logical_date}.csv','ARTIST_INFO_GLOBALTOP50')