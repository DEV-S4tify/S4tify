...
def read_and_merge():
    ...
    merged_df1 = pd.merge(global_top50_df, artist_info_df, on = 'artist_id')
    global_top50_artist_df = pd.merge(merged_df1, artist_top10_df, on='artist_id')
    
    file_path = f"data/spotify_join_data_{TODAY}.csv"
    
    global_top50_artist_df.to_csv(file_path, encoding='utf-8', mode='a', header=True, index=False)
